...
def parse_args():
    parser = argparse.ArgumentParser(description="GLEAM Stage2 migration entrypoint (Isaac Lab stack).")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to a Stage1/Stage2 saved *.zip checkpoint for Stage2 training.",
    )
    parser.add_argument(
        "--reset_num_timesteps",
        action="store_true",
...
def main():
    ...
    exp_name = "train_gleam_stage2_lab"
    seed = int(args.seed)
    ...
    if args.reset_num_timesteps:
        print(f"[TRAIN] Stage2 training for {additional_timesteps} steps (counter reset).")
    else:
        print(f"[RESUME] Continue Stage2 from {model.num_timesteps} steps for +{additional_timesteps} steps.")
