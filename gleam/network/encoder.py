...
class Encoder_GLEAM(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        encoder_param = copy.deepcopy(encoder_param)
        net_param = copy.deepcopy(net_param)
        self.map_channel = visual_input_shape[0]
        self.map_shape = visual_input_shape[1:]
        self.state_input_shape = state_input_shape
        append_hidden_shapes = list(net_param["append_hidden_shapes"])
        feature_dim = append_hidden_shapes[-1]
        net_param["append_hidden_shapes"] = append_hidden_shapes[:-1]
